...
//		printk("kevin ----> MXC_CCM_CBCMR_PRE_PERIPH_CLK_SEL_OFFSET === reg : %u\n", reg);
...
//printk("kevin ----> MXC_CCM_CBCDR_AHB_PODF_OFFSET === div : %u \n", div);
	return clk_get_rate(clk->parent) / div;
}
...
static int _clk_ahb_set_rate(struct clk *clk, unsigned long rate)
{
	...
	u32 parent_rate = clk_get_rate(clk->parent);
	div = parent_rate / rate;
//printk("kevin ----> _clk_ahb_set_rate    parent_rate = %d, rate=%d, div=%d \n", parent_rate, rate, div);
	if (div == 0)
		div++;
	if (((parent_rate / div) != rate) || (div > 8))
		return -EINVAL;
...
//printk("kevin ----> MXC_CCM_CBCDR_IPG_PODF_OFFSET  === div : %u\n", div);
	return clk_get_rate(clk->parent) / div;
}

//kevin test
static int _clk_ipg_set_rate(struct clk *clk, unsigned long rate)
{
        u32 reg, div;

//printk("kevin ----> _clk_ipg_set_rate \n");

        reg = __raw_readl(MXC_CCM_CBCDR);
        reg &= ~MXC_CCM_CBCDR_IPG_PODF_MASK;
        reg |= 0 << MXC_CCM_CBCDR_IPG_PODF_OFFSET;
        __raw_writel(reg, MXC_CCM_CBCDR);
...
static struct clk ipg_clk = {
	...
	.set_rate = _clk_ipg_set_rate, //kevin test
};
...
//printk("kevin ----> MXC_CCM_CSCMR1_PERCLK_PODF_OFFSET === div : %u \n", div);
...
static int _clk_ipg_perclk_set_rate(struct clk *clk, unsigned long rate)
{
	...
	u32 parent_rate = clk_get_rate(clk->parent);
	div = parent_rate / rate;
//printk("kevin ----> _clk_ipg_perclk_set_rate   parent_rate = %d , div = %d\n", parent_rate, div);
	if (div == 0)
		div++;
...
#define machine_for_PRJ_Winmate() machine_is_mx6q_sabresd() | machine_is_FA30() | machine_is_FA33() | machine_is_FA33_Signbox()//winmate frank add
	...
	unsigned long ckih1, unsigned long ckih2)
{
	 ...
	 */
//printk("kevin ----> mx6_clocks_init \n");
	clk_set_rate(&ipg_perclk, 66000000);		//kevin orig is 22000000
	clk_get_rate(&ipg_perclk);
//printk("kevin ----> mx6_clocks_init BBB\n");
	...
	*/
	...
	__raw_writel(1 << MXC_CCM_CCGRx_CG12_OFFSET |
		     ...
		     3 << MXC_CCM_CCGRx_CG4_OFFSET |			//kevin add for nfc
	...
	__raw_writel(1 << MXC_CCM_CCGRx_CG14_OFFSET |
		     ...
		     3 << MXC_CCM_CCGRx_CG6_OFFSET |	/* ldb_di0_clk */ //winmate frank add for ch0 clk enable
		     3 << MXC_CCM_CCGRx_CG2_OFFSET |	/* ipu1_di1_clk */
		     3 << MXC_CCM_CCGRx_CG1_OFFSET |	/* ipu1_di0_clk *///winmate frank add for Di0 clk enable		     
	...
	__raw_writel(3 << MXC_CCM_CCGRx_CG7_OFFSET |
			 ...
			 * backlight on.
			 */
//rudolph add for fa30 20140112
#if 0 //winmate frank mask
...
#endif
#else
			(machine_for_PRJ_Winmate() ?   //winmate frank add 
#endif
