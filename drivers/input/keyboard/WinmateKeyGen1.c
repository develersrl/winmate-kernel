...
#define KEYGEN1_MAJ 330

static int KeyGenDeviceEnum=0;
		 ...
		 loff_t *off)
	{
		...
		for(i=0;i<len;i++)
		{
			...
			if(pInput_dev)
			{
				if(cap)
				{
					input_event(pInput_dev, EV_MSC, MSC_SCAN, KEY_LEFTSHIFT);
					input_report_key(pInput_dev,KEY_LEFTSHIFT, 1);
				...
				{
					input_event(pInput_dev, EV_MSC, MSC_SCAN, KEY_LEFTSHIFT);
					input_report_key(pInput_dev,KEY_LEFTSHIFT, 0);
...
static struct file_operations KeyGen_fops = {
       write: KeyGenWrite,
       open: KeyGenOpen,
...
static void TimerRefresh(unsigned long a)
{
...
static int __devinit WinamteGenKey_probe(struct platform_device *pdev)
{
		...
		sprintf(pName,"WinmateVirtualKeyGen1");
		sprintf(pPhyName,"WinmateKeyGen1/input");
		...
		err=register_chrdev(KEYGEN1_MAJ, "keygen1",&KeyGen_fops);	
		if(err) 
		{
...
MODULE_DESCRIPTION("Virtual Key driver 1");
MODULE_LICENSE("GPL");
