...
#include <linux/gpio.h> // winmate frank add for panel 
#include <linux/io.h>
#include <linux/ipu.h>
...
#include "../../../../include/paneltype.h" //winmate frank add for panel seting
#include <linux/delay.h> //winmate frank add 
...
/*winmate frank add for Selet DI0/DI1 to LVDS0 or LVDS1*/
unsigned int  ldb_Channel_Set(void)
{
	enum ELvdsMode eLvdsMode=eChDisable;
	int pixel_fmt=PANEL_PIX_FMT;

	if((PANEL_LDB_MODE==LDB_SPL_DI0)||(PANEL_LDB_MODE==LDB_SPL_DI1)||(PANEL_LDB_MODE==LDB_DUL_DI0)||(PANEL_LDB_MODE==LDB_DUL_DI1))
	{
		
		if((PANEL_LDB_MODE==LDB_SPL_DI0)||(PANEL_LDB_MODE==LDB_SPL_DI1))
		{
			if(PANEL_LDB_MODE==LDB_SPL_DI0)
			{

				eLvdsMode = eCh1EnableToDI0 |eCh0EnableToDI0;
			}
			else
			{
			
				eLvdsMode = eCh0EnableToDI1 |eCh1EnableToDI1;
			
			}

				
			
			eLvdsMode |=eSplitModeEnable ;
		}			
		else
		{
			if(LDB_CANNEL_CHAGE)
				eLvdsMode = eCh0EnableToDI1 |eCh1EnableToDI0;
			else
				eLvdsMode = eCh0EnableToDI0 |eCh1EnableToDI1;
		
		}
			
		eLvdsMode |= (eDI0_VS|eDI1_VS);


	}
	else if((PANEL_LDB_MODE==LDB_SIN0)||(PANEL_LDB_MODE==LDB_SEP0))
	{
		if(LDB_CANNEL_CHAGE)
			eLvdsMode = eCh1EnableToDI0;
		else
			eLvdsMode = eCh0EnableToDI0;

	}
	else if((PANEL_LDB_MODE==LDB_SIN1)||(PANEL_LDB_MODE==LDB_SEP1))
	{
		if(LDB_CANNEL_CHAGE)
			eLvdsMode = eCh0EnableToDI1 ;
		else
			eLvdsMode = eCh1EnableToDI1;
...
//----------set pixel-------------------
	...
	switch (pixel_fmt) 
	{
		case IPU_PIX_FMT_BGR24:
		case IPU_PIX_FMT_RGB24:
			eLvdsMode |=(eCh0_24Bit|eCh1_24Bit);
		break;
#if 0		
		case IPU_PIX_FMT_BGR666:
		case IPU_PIX_FMT_RGB666:
	...
	if(PANEL_BIT_MAPPING)
	{
		eLvdsMode |= (eCh1_JEIDA|eCh0_JEIDA);
	}


	return (unsigned int )eLvdsMode;
...
//	printk(KERN_NOTICE "%s_%s(%d) winmate frank pixel_clk=[%d]\n",__FILE__,__func__,__LINE__,pixel_clk);	
...
#define BL_EN		IMX_GPIO_NR(1, 2)//  BLON_3V_H winmate frank
#define LCD_EN		IMX_GPIO_NR(6,15)// key_col6 winmate frank 
			   ...
			   struct fb_info *fbi)
{
	...
	index = find_ldb_setting(ldb, fbi);
	    //---winmate frank add for panel pwr on
	if(gpio_get_value(LCD_EN)==0)
	{
		gpio_set_value(LCD_EN,1);
		mdelay(PANEL_PS_T1);//winmant frank panel_pwr_on to  lvds_signal_on time
	}
	    //---winmate frank add for panel pwr on
	if (index < 0)
		return index;
	...
	reg &=~0xff;
	reg |= ldb_Channel_Set();
	//printk(KERN_NOTICE "%s_%s(%d) winmate frank reg=[%x] PANEL_NAME=[%s]\n",__FILE__,__func__,__LINE__,reg,PANEL_NAME);	
	writel(reg, ldb->control_reg);
	mdelay(PANEL_PS_T2);//winmant frank lvds_signal on to BL_PWR_on time
			     ...
			     struct fb_info *fbi)
{
	...
	mdelay(PANEL_PS_T2);//winmant frank BL_PWR_off to lvds_signal_off   time
	index = find_ldb_setting(ldb, fbi);
	if (index < 0)
		return;

	reg = readl(ldb->control_reg);
	//winmate frank mask reg &= ~ldb->setting[index].ch_mask;
	reg &= ~0xf;
	writel(reg, ldb->control_reg);
	mdelay(PANEL_PS_T1);//winmant frank lvds_signal off to panel_pwr off time
	gpio_set_value(LCD_EN,0); //panel pwr off
}
	...
	struct mxc_dispdrv_setting *setting)
{
	...
	if (!ldb->inited) {
		...
		//printk(KERN_NOTICE "%s_%s(%d) winmate frank debug panel logo to lvds0 reg=[%x]\n",__FILE__,__func__,__LINE__,reg);	
		reg &=~0xff;//winmat frank add
		reg |= ldb_Channel_Set();//winmat frank add // 1:disable channel out 0: enable channel out
		//printk(KERN_NOTICE "%s_%s(%d) winmate frank reg=[%x] PANEL_NAME=[%s]\n",__FILE__,__func__,__LINE__,reg,PANEL_NAME);	
		writel(reg, ldb->control_reg);
