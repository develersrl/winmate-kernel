...
timeout = jiffies + HZ / 10000;	//kevin 500 ms
...
//		last_jiffies = jiffies;              //kevin test
		dev_dbg(&i2c_imx->adapter.dev,
		...
		if (result)
			return result;
		result = i2c_imx_acked(i2c_imx);
//		current_jiffies = jiffies;		//kevin test
		if (result)
			return result;
		
		/*if((last_jiffies - current_jiffies)>timeout){
			printk("kevin ----> I2C error !!!  last_jiffies : %lu  , current_jiffies :%lu\n", last_jiffies, current_jiffies);
		}*/
	}
/*
	atomic_set(&v, 0);
