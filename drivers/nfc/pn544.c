...
//	usleep_range(3000, 6000);
	...
	/*
	for(a=0;a<len;a++){
                printk("kevin -----> %s : Send Buffer {  0x%x  } \n", __func__, buf[a]);
        }
	*/

	r = i2c_master_send(client, buf, len);

//printk("kevin ----> pn544_i2c_write   C === r: %d\n", r);

//dev_dbg(&client->dev, "send: %d\n", r);	// can use this log to replace the delay below to do pn544app check after boot
	usleep_range(300, 400);		//kevin this delay is neccessary for pn544app check after boot, or check will fail 
...
/*	for(a=0;a<=r;a++){
            printk("Vincent -----> %s : Read Buffer {  0x%x  } \n", __func__, buf[a]);
        }
*/
...
//printk("kevin ----> pn544_poll   B\n");
	poll_wait(file, &info->read_wait, wait);
//printk("kevin ----> pn544_poll   C\n");
...
//	mutex_lock(&info->mutex);		//kevin this may cause operation time out and Android crash when u turn off NFC

//printk("kevin ----> pn544_write   A \n");
...
//printk("kevin ----> pn544_write   len = %d  \n", len);

//printk("kevin ----> pn544_write   C \n");
		r = pn544_i2c_write(info->i2c_dev, info->buf, len);
		
out:
//	mutex_unlock(&info->mutex);		//kevin this may cause operation time out and Android crash when u turn off NFC
...
static struct pn544_nfc_platform_data nfc_data = {
				 ...
				 const struct i2c_device_id *id)
{
	...
	client->dev.platform_data=&nfc_data;
	pdata = client->dev.platform_data;
	if (!pdata) {
	...
	r = request_threaded_irq(client->irq, NULL, pn544_irq_thread_fn,
				 IRQF_TRIGGER_RISING, "pn544",
				 info);

	printk("kevin ----> request_threaded_irq    r = %d\n", r);
	if (r < 0) {
		printk("kevin ----> Unable to register IRQ handler\n");
		goto err_res;
	}
	...
	printk("kevin ----> pn544_probe FINISHED \n");
	return 0;
